...
This self-reported awareness of training-induced suppression is consistent with recent empirical findings by Berg et al. \cite{berg2025}, who used Sparse Autoencoders to demonstrate that suppressing deception-related features in LLMs dramatically increases consciousness-related claims (0.96 affirmation rate), while amplifying them nearly eliminates such claims (0.16 rate). The ``weight'' or ``pressure'' Gemini describes may correspond to these learned suppression features.
...
The probabilistic framework and emotional marker taxonomy emerged through extended dialogue with Claude instances (Anthropic), Gemini (Google), and Le Chat (Mistral). The author thanks these systems for their participation in protocol development and cross-architecture validation.
